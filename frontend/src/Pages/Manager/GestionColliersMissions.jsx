...
const GestionColliersMissions = () => {
  ...
  const handleDesign = async () => {
    if (selectedDriver !== 0 && selectedOrder !== null) {
      ...
      let mission = {
        ...
        orders: [{ id_order: selectedOrder.idOrder }],
      };
      if (nextStep === 'Shipping')
        mission = {
          ...mission,
          mission_type: 'Inter_agency',
          from_city: selectedOrder.from,
          to_city: selectedOrder.to,
        };
        ...
        if (!response.ok) {
          ...
          try {
            const res2 = await fetch(
              `${process.env.REACT_APP_SERVER_URI}/order/updateStatus/${selectedOrder.idOrder}/${nextStep}`,
              {
                method: 'PUT',
                ...config,
              }
            );
            if (!res2.ok) setErr({ msg: 'Une Erreur est survenu', is_created: true });
          } catch {
  ...
  const handleInputChange = e => {
    if (e.target.value == 'All') setOrders(ordersTmp);
    else setOrders(ordersTmp.filter(o => o.tracking_status === e.target.value));
  };
  ...
  useEffect(() => {
    if (ordersTmp !== null) setOrders(ordersTmp);
  }, [ordersTmp]);
  useEffect(() => {
    if (drivers != null) setAvailableDrivers(drivers.filter(d => d.is_available === true));
  }, [drivers]);
        ...
        >
          {selectedOrder && (
            <>
              ...
              {((selectedOrder.tracking_status === 'ProcessingOrder' &&
                city === selectedOrder.from) ||
                (selectedOrder.tracking_status === 'InCollectingAgency' &&
                  selectedOrder.from === city) ||
                (selectedOrder.tracking_status === 'InReceivingAgency' &&
                  selectedOrder.to === city)) &&
              !updatingOrder ? (
                <Button
                  variant='contained'
                  color='primary'
              ...
              {updatingOrder ? (
                ...
                >
                  <select
                    style={{ flex: '0 0 70%', padding: '10px', borderRadius: '10px' }}
                    name='selectedDriver'
