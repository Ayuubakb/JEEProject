...
const ManagerProfile = () => {
  ...
  const handleModify = async () => {
    ...
    if (!response.ok) setErr({ err: await response.text(), updated: false });
    else {
  ...
  useEffect(() => {
    if (toModify === true) setErr({ err: null, updated: false });
  }, [toModify]);
  ...
  return (
    <section className='profile'>
      <div class='bienvenu'>
        <h1>
          Bienvenue,<br></br> {infos.first_name} {infos.last_name}
        </h1>
        {!toModify ? (
          <button
      ...
      {err.err !== null ? (
        <p
          className='err'
          style={{ backgroundColor: err.updated ? 'blue' : 'red' }}
        >
          {err.err}
        </p>
      ) : null}
      ...
      <div className='agencyInfo'>
        <div>
          ...
          {!toModify ? (
            <p>
              {userInfos?.city} - {agencies.filter(a => a.city === userInfos?.city)[0]?.address}
            </p>
          ) : (
            <select
              name='city'
              ...
              onChange={e => {
                setInfos(prev => {
                  return { ...prev, agency: { city: e.target.value } };
                });
              }}
            >
              {agencies.map(a => {
