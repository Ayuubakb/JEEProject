...
public class ClientController {
    ...
    public ResponseEntity<String> addClient(@RequestBody Client client) {
        ...
        if (!mailExists) {
            try {
                agency = agencyRepository.findById(client.getAgency().getId_agency());
            } catch (Exception e) {
    ...
    public ResponseEntity<String> updateClient(@RequestBody Client newClient) {
        ...
        try {
            clientOpt = clientRepo.findById(newClient.getId_user());
        } catch (Exception e) {
            return stringInternalError;
        }
        ...
        if (!existingClient.getEmail().equals(newClient.getEmail()) && utils.mailExists(newClient.getEmail(), newClient.getId_user()))
            return new ResponseEntity<>("Email Already Used", HttpStatus.BAD_REQUEST);
        ...
        // Update fields
        existingClient.setFirst_name(newClient.getFirst_name());
        existingClient.setLast_name(newClient.getLast_name());
        existingClient.setEmail(newClient.getEmail());
        existingClient.setCompany(newClient.getCompany());
        existingClient.setAddress(newClient.getAddress());
        existingClient.setAgency(agency.get());
